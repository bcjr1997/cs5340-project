...
class VAE(nn.Module):
    def __init__(self, image_dim, input_channels=1, latent_dim=256):
        ...
        self.encoder_l2 = nn.Sequential(nn.Conv2d(32, 32 * 2, kernel_size=4, stride=2, padding=1), nn.ReLU(), nn.Dropout(0.1))
        self.encoder_l3 = nn.Sequential(nn.Conv2d(32 * 2, 32 * 4, kernel_size=4, stride=2, padding=1), nn.ReLU(), nn.Dropout(0.2))
        self.encoder_l4 = nn.Sequential(nn.Conv2d(32 * 4, 32 * 8, kernel_size=4, stride=2, padding=1), nn.ReLU(), nn.Dropout(0.3))
        self.encoder_l5 = nn.Sequential(nn.Conv2d(32 * 8, 32 * 16, kernel_size=4, stride=2, padding=1), nn.ReLU(), nn.Dropout(0.4))
        ...
        self.decoder_l2 = nn.Sequential(nn.ConvTranspose2d(32 * 16, 32 * 4, kernel_size=4, stride=2, padding=1), nn.ReLU(), nn.Dropout(0.4))
        self.decoder_l3 = nn.Sequential(nn.ConvTranspose2d(32 * 8, 32 * 2, kernel_size=4, stride=2, padding=1), nn.ReLU(), nn.Dropout(0.2))
        self.decoder_l4 = nn.Sequential(nn.ConvTranspose2d(32 * 4, 32, kernel_size=4, stride=2, padding=1), nn.ReLU(), nn.Dropout(0.1))
